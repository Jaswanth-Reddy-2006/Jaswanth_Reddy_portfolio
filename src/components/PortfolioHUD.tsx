            ...
            <div className="hidden lg:flex flex-col items-center gap-2 flex-1 max-w-sm px-12">
                <div className="w-full h-[2px] bg-white/5 relative overflow-hidden rounded-full">
                    <motion.div
                        ...
                        className={`absolute inset-y-0 left-0 transition-colors duration-700 ${isSalaarMode ? 'bg-red-600 shadow-[0_0_10px_#ff0000]' : 'bg-mutedBlue shadow-[0_0_10px_rgba(51,102,204,0.5)]'}`}
                    />
                </div>
                <div className="flex justify-center w-full text-[9px] font-mono text-lightText/40 uppercase tracking-[0.3em]">
                    <span className={`font-bold transition-colors duration-700 ${isSalaarMode ? 'text-red-600' : 'text-mutedBlue'}`}>{displayPercent}% {isSalaarMode ? 'OBJECTIVE PROGRESS' : 'Complete'}</span>
                </div>
            </div>
